...
#ifndef CTM_MATH_VDIMS
#define CTM_MATH_VDIMS const int& dimensions = this->dimensions
#endif

#ifndef CTM_MATH_VNEWV
#define CTM_MATH_VNEWV(ma0)\
ma0* vectormath::newVector(const ma0&)\
{\
	return (ma0*)calloc(this->dimensions, sizeof(ma0));\
}
...
#ifndef CTM_MATH_VADDT
#define CTM_MATH_VADDT(ma0)\
ma0* vectormath::add(ma0* out, ma0* vector, ma0* vector1)\
{\
	CTM_MATH_VDIMS;\
	for(int i = 0; i < dimensions; i++)\
		out[i] = vector[i] + vector1[i];\
	return out;\
}
...
#ifndef CTM_MATH_VSUBT
#define CTM_MATH_VSUBT(ma0)\
ma0* vectormath::subtract(ma0* out, ma0* vector, ma0* vector1)\
{\
	CTM_MATH_VDIMS;\
	for(int i = 0; i < dimensions; i++)\
		out[i] = vector[i] - vector1[i];\
...
#ifndef CTM_MATH_VSCLR
#define CTM_MATH_VSCLR(ma0, ma1)\
ma0* vectormath::scalar(ma0* out, ma1* vector, ma0 scalar)\
{\
	CTM_MATH_VDIMS;\
	for(int i = 0; i < dimensions; i++)\
...
#ifndef CTM_MATH_VMLTP
#define CTM_MATH_VMLTP(ma0, ma1)\
ma0* vectormath::multiply(ma0* out, ma1* vector, ma1* vector1)\
{\
	CTM_MATH_VDIMS;\
	for(int i = 0; i < dimensions; i++)\
		out[i] = (ma0)vector[i] * vector1[i];\
	return out;\
}
...
#ifndef CTM_MATH_VMGNT
#define CTM_MATH_VMGNT(ma0, ma1, ma2)\
ma0 vectormath::magnitude(const ma0&, ma1* vector)\
{\
	return sqrtf((ma0)this->squareMagnitude((ma2)0, vector));\
}
...
#ifndef CTM_MATH_VSMGT
#define CTM_MATH_VSMGT(ma0, ma1)\
ma0 vectormath::squareMagnitude(const ma0&, ma1* vector)\
{\
	CTM_MATH_VDIMS;\
	ma0 sm = 0;\
	for(int i = 0; i < dimensions; i++)\
		sm += (ma0)vector[i] * vector[i];\
	return sm;\
}
